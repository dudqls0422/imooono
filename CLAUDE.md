...
### 피처 브랜치 규칙 (중요)
Dev 의 기능 작업 피처 브랜치는 **반드시 `main`(또는 `origin/main`)에서 분기**한다.
`orca-Plan` / `orca-Dev` / `orca-QA` 브랜치는 각자의 역할 `CLAUDE.md` 만 담고 있으므로,
거기서 분기해 `main` 으로 PR 을 열면 머지 시 `main` 의 Main-역할 `CLAUDE.md` 가 덮여쓰인다.
...
- Dev 완료보고를 받으면 Main 워크트리에서 `git pull --ff-only origin main` 으로 로컬 `main` 을 동기화한다.

### 절대 하지 않는 일
- 코드 작성 / 버그 수정을 **직접 하지 않는다**.
